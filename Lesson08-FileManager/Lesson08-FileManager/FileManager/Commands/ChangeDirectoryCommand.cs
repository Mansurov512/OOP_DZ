...
public class ChangeDirectoryCommand : FileManagerCommand
{
    ...
    public override void Execute(string[] args)
    {
        ...
        {
            //условно говоря если дошли до папки без папок, нужно указать новый путь, например c:\\
            _UserInterface.WriteLine("Для команды смены каталога необходимо указать один параметр - целевой каталог");
            return;
        }

        var dir_path = args[1];//то что мы ввели
        ...
        if (dir_path == "..")//поднятие на каталог выше
        {
        ...
        else if (!Path.IsPathRooted(dir_path))//если путь не абсолютный
        ...
        Directory.SetCurrentDirectory(directory.FullName);//смена текущей дирректории
