...
public class ConsoleUserInterface : IUserInterface //методы
{
    ...
    private void WritePrompt(string? Prompt, bool PromptNewLine)
    {
        ...
        if (Prompt is { Length: > 0 })
            if (PromptNewLine)
                WriteLine(Prompt);//перенос курсора на новую строку
            else
                Write(Prompt);//или нет
    }
